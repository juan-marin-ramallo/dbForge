...
BEGIN  
 ...
 BEGIN  
  ...
  INNER JOIN 
		@Data AS P ON PR.[Name] = P.[Name]  /*AND PR.[IdUser] = P.[IdUser]*/ --Se quita para que en la importacion masiva se actualice el de cualquier usuario
  WHERE PR.[Deleted] = 0  
...
END 
 ...
 BEGIN
  ...
  FROM	[dbo].[ProductCategory] PR
  LEFT OUTER JOIN 
		@Data AS P ON PR.[Name] = P.[Name]  /*AND PR.[IdUser] = P.[IdUser]*/ --Se quita para que en la importacion masiva se elimine el de cualquier usuario que no coincida en la template
  WHERE P.[Name] IS NULL AND PR.[Deleted] = 0  
  ...
  LEFT OUTER JOIN 
		@Data AS P ON PR.[Name] = P.[Name] /*AND PR.[IdUser] = P.[IdUser]*/ --Se quita para que en la importacion masiva se elimine el de cualquier usuario que no coincida en la template
  WHERE P.[Name] IS NULL AND PR.[Deleted] = 1  	
 ...
 SELECT P.[Name],P.[Description],IIF(@OrderByName = 1, NULL, P.[Order]) [Order]
 FROM @Data P  
 ...
 BEGIN   
	  ...
	  FROM    @Data P  
	  LEFT OUTER JOIN [dbo].[ProductCategory] PR WITH (NOLOCK) ON PR.[Name] = P.[Name] /*AND PR.[IdUser] = P.[IdUser]*/ --Se quita para que en la importacion masiva se agregue asi si no existe asi haya sido creado por otro usuario
	  WHERE   PR.[Id] IS NULL 
